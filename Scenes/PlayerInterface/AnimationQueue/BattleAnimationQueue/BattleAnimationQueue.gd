...
func animate_status(character_data:CharacterData, status_data:StatusData, delta:int, anim_type:int = 1, wait_time:float = 0.3):
...
func animate_move(card_data:CardData, new_transform:TransformData, anim_type:int = 0, tween_time:float = get_tween_time(), wait_time:float = get_wait_time()):
