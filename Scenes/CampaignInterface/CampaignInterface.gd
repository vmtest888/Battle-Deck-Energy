...
@export var starting_player_data: Resource = preload("res://Resources/Characters/Player/NewCampaignPlayerData.tres")
@export var starting_deck_data: Resource = preload("res://Resources/Decks/LamestStartingDeck.tres")
...
func start_credits():
	battle_shadow_panel.show()
	var credits_interface = credits_panel_scene.instantiate()
	campaign_interface_container.add_child(credits_interface)
	credits_interface.connect("continue_pressed", Callable(self, "_start_next_level"))

func _on_save_deck(cards : Array, title : String, icon : Texture2D):
	PersistentData.save_deck(cards, title, icon)
	_start_next_level()

func save_deck():
	battle_shadow_panel.show()
	var save_deck_interface = save_deck_panel_scene.instantiate()
...
func _start_next_level():
	...
	await level_delay_timer.timeout
